...
function KontrolGecmisiModal({
  ...
  const durumRenk: Record<EkipmanStatus, { color: string; bg: string; border: string; icon: string }> = {
    'Uygun':       { color: '#059669', bg: 'rgba(5,150,105,0.1)',   border: 'rgba(5,150,105,0.2)',   icon: 'ri-checkbox-circle-fill' },
    'Uygun Değil': { color: '#DC2626', bg: 'rgba(220,38,38,0.1)',   border: 'rgba(220,38,38,0.2)',   icon: 'ri-close-circle-fill' },
    'Bakımda':     { color: '#D97706', bg: 'rgba(217,119,6,0.1)',   border: 'rgba(217,119,6,0.2)',   icon: 'ri-tools-fill' },
    'Hurda':       { color: '#64748B', bg: 'rgba(100,116,139,0.1)', border: 'rgba(100,116,139,0.2)', icon: 'ri-delete-bin-fill' },
  };

  return (
    <Modal isOpen={open} onClose={onClose} title="Kontrol Geçmişi" size="sm" icon="ri-history-line"
      footer={<button onClick={onClose} className="btn-secondary whitespace-nowrap w-full">Kapat</button>}
    >
      <div>
        <div className="px-3 py-2 rounded-xl mb-4" style={{ background: 'rgba(51,65,85,0.3)', border: '1px solid rgba(51,65,85,0.4)' }}>
          <p className="text-xs" style={{ color: '#64748B' }}>Ekipman</p>
          <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{ekipmanAd}</p>
        </div>
        {loading ? (
          <div className="flex flex-col items-center justify-center py-10 gap-3">
            <div className="w-6 h-6 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'rgba(5,150,105,0.3)', borderTopColor: '#059669' }} />
            <p className="text-xs" style={{ color: '#475569' }}>Yükleniyor...</p>
          </div>
        ) : gecmis.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-3">
            <div className="w-12 h-12 flex items-center justify-center rounded-2xl" style={{ background: 'rgba(51,65,85,0.3)', border: '1px solid rgba(51,65,85,0.4)' }}>
...
function EvrakModal({
  ...
  return (
    <Modal isOpen={open} onClose={onClose} title={dosyaAdi || 'Belge'} size="lg" icon="ri-file-text-line"
      footer={<><button onClick={onClose} className="btn-secondary whitespace-nowrap">Kapat</button><button onClick={handleDownload} className="btn-primary whitespace-nowrap"><i className="ri-download-2-line mr-1" />İndir</button></>}
    >
      <div className="flex flex-col items-center gap-4">
        ...
        {!isImage && !isPdf && (
          <div className="text-center py-8">
            <div className="w-16 h-16 flex items-center justify-center rounded-2xl mx-auto mb-4" style={{ background: 'rgba(5,150,105,0.1)', border: '1px solid rgba(5,150,105,0.2)' }}>
