...
const RISK_SEVIYELERI = ['Düşük Tehlikeli', 'Tehlikeli', 'Çok Tehlikeli'];
...
export default function SaglikGuvenlikPlaniModal({ onClose }: Props) {
  ...
  const [riskSeviyesi, setRiskSeviyesi] = useState('');
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { data: fnData, error: fnErr } = await supabase.functions.invoke('openai-assistant', {
        body: { mode: 'saglik-guvenlik-plani', data: { sektor, firmaAdi, calisanSayisi, riskSeviyesi, prompt } },
      });
  ...
  const handlePdfExport = async () => {
    ...
    setPdfLoading(true);
    try {
      await exportToPDF(contentRef.current, `SGP_${firmaAdi || 'Plan'}.pdf`);
    } finally {
      setPdfLoading(false);
    }
  };
      ...
      <div className="w-full rounded-2xl flex flex-col overflow-hidden"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b flex-shrink-0" style={{ borderColor: 'var(--border-main)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Sağlık Güvenlik Planı</h2>
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>AI Destekli — Yasal Uyumlu</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {step === 'form' ? (
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={calisanSayisi} onChange={e => setCalisanSayisi(e.target.value)} placeholder="Örn: 120"
                    className="w-full rounded-xl px-4 py-2.5 text-sm outline-none"
                    style={{ background: 'var(--bg-app)', border: '1px solid var(--border-main)', color: 'var(--text-primary)' }} />
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Tehlike Sınıfı</label>
                <div className="flex gap-2">
                  {RISK_SEVIYELERI.map(r => (
                    <button key={r} onClick={() => setRiskSeviyesi(r)} className="flex-1 text-xs py-2 px-3 rounded-lg cursor-pointer transition-all"
                      style={{ background: riskSeviyesi === r ? 'rgba(5,150,105,0.08)' : 'var(--bg-app)', border: `1px solid ${riskSeviyesi === r ? 'rgba(5,150,105,0.35)' : 'var(--border-main)'}`, color: riskSeviyesi === r ? '#059669' : 'var(--text-muted)', fontWeight: riskSeviyesi === r ? 600 : 400 }}>
                      {r}
                    </button>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {SEKTORLER.map(s => (
                    <button key={s} onClick={() => setSektor(s)} className="text-xs py-2 px-3 rounded-lg text-left cursor-pointer transition-all overflow-hidden text-ellipsis whitespace-nowrap"
                      style={{ background: sektor === s ? 'rgba(5,150,105,0.08)' : 'var(--bg-app)', border: `1px solid ${sektor === s ? 'rgba(5,150,105,0.35)' : 'var(--border-main)'}`, color: sektor === s ? '#059669' : 'var(--text-muted)', fontWeight: sektor === s ? 600 : 400 }}>
                      {s}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea value={prompt} onChange={e => setPrompt(e.target.value)}
                  placeholder="Örn: Gece vardiyası çalışması var, buna özel maddeler ekle" rows={3}
            ...
            <div ref={contentRef} className="p-6 space-y-4">
              {ozet && (
                <div className="rounded-xl p-4" style={{ background: 'rgba(5,150,105,0.05)', border: '1px solid rgba(5,150,105,0.15)' }}>
                  <p className="text-xs font-semibold mb-2" style={{ color: '#059669' }}>Plan Özeti</p>
              ...
              {sections.map((section, si) => (
                <div key={si} className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border-main)' }}>
                  <div className="px-4 py-3 flex items-center gap-2" style={{ background: 'var(--bg-app)', borderBottom: '1px solid var(--border-main)' }}>
                    <span className="w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0" style={{ background: '#059669' }}>{si + 1}</span>
                  ...
                  <ul className="p-4 space-y-2">
                    {section.icerik.map((item, ii) => (
                      <li key={ii} className="flex items-start gap-2 group">
                        <span className="w-1.5 h-1.5 rounded-full mt-2 flex-shrink-0" style={{ background: '#059669' }} />
                    ...
                    <li>
                      <button onClick={() => addItem(si)} className="flex items-center gap-1.5 text-[11px] cursor-pointer mt-1 px-2 py-1 rounded-lg transition-all"
                        ...
                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#059669'; (e.currentTarget as HTMLElement).style.borderColor = 'rgba(5,150,105,0.3)'; }}
          ...
          {step === 'result' ? (
            <>
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={handlePdfExport} disabled={pdfLoading}
                  ...
                  style={{ background: pdfLoading ? 'rgba(5,150,105,0.4)' : '#059669', color: '#fff', opacity: pdfLoading ? 0.7 : 1 }}>
            ...
            <>
              ...
              <button onClick={handleGenerate} disabled={!sektor || !firmaAdi.trim() || loading}
                ...
                style={{ background: !sektor || !firmaAdi.trim() || loading ? 'rgba(5,150,105,0.4)' : '#059669', color: '#fff', opacity: !sektor || !firmaAdi.trim() || loading ? 0.6 : 1 }}>
