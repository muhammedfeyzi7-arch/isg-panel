...
function saHeaders(extra?: Record<string, string>) {
...
const TYPE_LABELS: Record<string, string> = {
  bug: 'Hata', feature: 'Özellik', question: 'Soru', other: 'Diğer',
};
...
export default function SupportTicketsPanel() {
  ...
  const fetchTickets = useCallback(async () => {
    setLoading(true);
    try {
      const data: Ticket[] = await saFetch('support_tickets?select=*&order=created_at.desc') ?? [];
      const orgIds = [...new Set(data.map(t => t.organization_id).filter(Boolean))] as string[];
      let orgMap: Record<string, string> = {};
      if (orgIds.length > 0) {
        const orgs: { id: string; name: string }[] = await saFetch(
          `organizations?id=in.(${orgIds.join(',')})&select=id,name`
        ) ?? [];
  ...
  const handleReply = async () => {
    ...
    setSending(true);
    try {
      await saFetch(`support_tickets?id=eq.${selected.id}`, {
        method: 'PATCH',
        body: JSON.stringify({
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          {openCount > 0 && (
            <span className="px-2 py-0.5 bg-red-50 text-red-600 text-xs font-medium rounded-md">{openCount} açık</span>
          )}
          ...
          <div className="flex gap-0.5 bg-white border border-slate-200 rounded-lg p-1">
            {(['all', 'open', 'replied'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-3 py-1 text-xs rounded-md font-medium transition-colors cursor-pointer whitespace-nowrap ${filter === f ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-700'}`}>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100">
          {filtered.map(ticket => (
            <div key={ticket.id} onClick={() => { setSelected(ticket); setReply(ticket.admin_reply || ''); }}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${ticket.status === 'open' ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-500'}`}>
                ...
                <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                  <span className="px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded text-xs">{TYPE_LABELS[ticket.issue_type] ?? 'Diğer'}</span>
          ...
          <div className="fixed right-0 top-0 h-full w-full md:max-w-lg bg-white border-l border-slate-200 z-50 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4 bg-[#f8fafc]">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="px-2 py-0.5 bg-white border border-slate-200 text-slate-600 text-xs rounded-md">{TYPE_LABELS[selected.issue_type] ?? 'Diğer'}</span>
                <span className={`px-2 py-0.5 text-xs rounded-md font-medium ${selected.status === 'open' ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-700'}`}>
              ...
              {selected.admin_reply && (
                <div className="bg-white rounded-xl border border-slate-200 p-4">
                  ...
                  {selected.replied_at && <p className="text-slate-400 text-xs mt-2">{new Date(selected.replied_at).toLocaleString('tr-TR')}</p>}
                </div>
              )}
              <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
                <p className="text-slate-600 text-xs font-medium">{selected.admin_reply ? 'Yanıtı güncelle' : 'Yanıt yaz'}</p>
