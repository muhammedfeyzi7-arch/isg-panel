...
export default function CopKutusuTab({ orgId, isDark, onFirmaRestored }: CopKutusuTabProps) {
  ...
  /** Firmaya atanmış uzman/hekimlerin atamalarından bu firmayı kaldır */
  const temizleAtamalar = async (firmaId: string) => {
    try {
      // Bu firmaya atanmış uzman/hekimleri bul
      const { data: atananlar } = await supabase
        .from('user_organizations')
        .select('user_id, active_firm_id, active_firm_ids')
        .eq('organization_id', orgId)
        .or(`active_firm_id.eq.${firmaId},active_firm_ids.cs.{"${firmaId}"}`);

      if (!atananlar || atananlar.length === 0) return;

      await Promise.all(
        atananlar.map(async (u) => {
          const yeniFirmIds = (Array.isArray(u.active_firm_ids) ? u.active_firm_ids : []).filter(
            (id: string) => id !== firmaId
          );
          const yeniActiveFirmId = u.active_firm_id === firmaId
            ? (yeniFirmIds[0] ?? null)
            : u.active_firm_id;
          ...
          return supabase
            .from('user_organizations')
            .update({
              active_firm_id: yeniActiveFirmId,
              active_firm_ids: yeniFirmIds.length > 0 ? yeniFirmIds : null,
            })
      ...
      console.warn('[CopKutusu] temizleAtamalar hata:', err);
    }
  };
  ...
  const handleGeriAl = async (firmaId: string, firmaAdi: string) => {
    ...
    try {
      ...
      const basarili = Array.isArray(updated) && updated.length > 0 && updated[0].deleted_at === null;
      if (!basarili) {
        showMesaj('error', 'Güncelleme yapılamadı. Lütfen sayfayı yenileyip tekrar deneyin.');
        return;
      }

      // Yerel listeden çıkar — kullanıcı çöp sekmesinde kalır
      setFirmalar(prev => prev.filter(f => f.id !== firmaId));
      showMesaj('success', `"${temizAd}" başarıyla geri alındı! Firmalar sekmesinden görebilirsiniz.`);

      // Üst listeyi arka planda yenile (sekme değişikliği YOK)
  ...
  const handleKaliciSilOnayla = async (firmaId: string) => {
    ...
    setKaliciSilVeriOzet({
      personelCount: 0, evrakCount: 0, ekipmanCount: 0,
    ...
    try {
      ...
      setKaliciSilVeriOzet({
        personelCount: 0, evrakCount: 0, ekipmanCount: 0,
        uygunsuzlukCount: 0, egitimCount: 0, loading: false,
      });
    }
  };
  ...
  // Kalıcı silme onay modalı — portal ile body'ye render et (sayfa kayması önlenir)
  const kaliciSilModal = kaliciSilOnayId
    ? (() => {
        const firma = firmalar.find(f => f.id === kaliciSilOnayId);
        if (!firma) return null;
        const ozet = kaliciSilVeriOzet;
        const islem = islemLoading === kaliciSilOnayId;
        const toplamVeri = ozet
          ? ozet.personelCount + ozet.evrakCount + ozet.ekipmanCount + ozet.uygunsuzlukCount + ozet.egitimCount
          : 0;
        return createPortal(
          <div
            className="fixed inset-0 flex items-center justify-center p-4"
            style={{ background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(14px)', zIndex: 99999 }}
            onClick={e => {
              if (e.target === e.currentTarget && !islem) {
                setKaliciSilOnayId(null);
                setKaliciSilVeriOzet(null);
              }
            }}
            ...
            >
              ...
              <div className="p-6">
                ...
                <p className="text-sm font-semibold mb-4" style={{ color: textPrimary }}>
                  ...
                  firmasını kalıcı olarak silmek üzeresiniz.
                </p>
                ...
                {ozet?.loading ? (
                  ...
                  <div className="rounded-xl p-4 mb-4" style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)' }}>
                    <p className="text-xs font-bold mb-3" style={{ color: '#EF4444' }}>
                      Bu firmaya ait aşağıdaki veriler orphan (sahipsiz) kalacak:
                    </p>
                  ...
                  >
                    ...
                    <span className="text-xs" style={{ color: '#16A34A' }}>
                      Bu firmaya ait kayıt bulunmuyor, güvenle silinebilir.
                    </span>
                  </div>
                )}

                {/* Atama temizleme uyarısı */}
                <div
                ...
                >
                  ...
                  <p className="text-xs" style={{ color: textMuted }}>
                    Bu firmaya atanmış <strong style={{ color: '#D97706' }}>uzman/hekim atamaları</strong> otomatik olarak temizlenecektir.
                  </p>
                </div>

                <div className="flex items-center gap-2 justify-end">
                  <button
                    onClick={() => { setKaliciSilOnayId(null); setKaliciSilVeriOzet(null); }}
                    disabled={islem}
                  ...
                  <button
                    onClick={() => void handleKaliciSil(firma.id, firma.name)}
                    disabled={islem || ozet?.loading}
                    ...
                    style={{
                      ...
                      opacity: (islem || ozet?.loading) ? 0.7 : 1,
                    }}
                  >
                    {islem ? (
                      <><i className="ri-loader-4-line animate-spin" />Siliniyor...</>
                    ) : (
                      <><i className="ri-delete-bin-line" />Kalıcı Olarak Sil</>
                    )}
                  </button>
                </div>
              </div>
    ...
    <div className="space-y-4 page-enter">
      ...
      >
        ...
        <span
          ...
          style={{
            background: firmalar.length > 0 ? 'rgba(239,68,68,0.1)' : 'rgba(100,116,139,0.1)',
            color: firmalar.length > 0 ? '#EF4444' : '#64748B',
            border: `1px solid ${firmalar.length > 0 ? 'rgba(239,68,68,0.2)' : 'rgba(100,116,139,0.15)'}`,
          }}
        >
      ...
      {mesaj && (
        ...
        >
          <i
            className={`${mesaj.tip === 'success' ? 'ri-checkbox-circle-line' : 'ri-error-warning-line'} text-sm`}
            style={{ color: mesaj.tip === 'success' ? '#22C55E' : '#EF4444' }}
          />
          <span
            className="text-xs font-semibold"
            style={{ color: mesaj.tip === 'success' ? '#16A34A' : '#DC2626' }}
          >
            {mesaj.metin}
          </span>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-1.5">
          {/* Tablo başlıkları */}
          <div
            className="hidden md:grid grid-cols-[2fr_1.5fr_1.5fr_180px] items-center px-4 py-2"
          ...
          {firmalar.map(f => {
            ...
            return (
              ...
              >
                ...
                {/* İşlemler */}
                <div
                  className="flex items-center gap-2 justify-start md:justify-end flex-wrap"
      ...
      {/* Kalıcı sil modal — portal (sayfa kayması olmaz) */}
      {kaliciSilModal}
    </div>
  );
}
