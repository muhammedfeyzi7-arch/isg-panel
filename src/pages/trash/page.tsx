...
type Tab = 'firmalar' | 'personeller' | 'evraklar';
...
export default function CopKutusuPage() {
  ...
  const totalDeleted = deletedFirmalar.length + deletedPersoneller.length + deletedEvraklar.length;
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 rounded-xl isg-card" style={{ width: 'fit-content' }}>
        {tabs.map(tab => (
          <button
            key={tab.id}
...
function TrashEmpty({ type }: { type: string }) {
  const labels: Record<string, string> = { firma: 'Çöp kutusunda firma yok', personel: 'Çöp kutusunda personel yok', evrak: 'Çöp kutusunda evrak yok' };
  return (
    <div className="flex flex-col items-center py-16 gap-3">
      ...
      <p className="text-sm font-medium" style={{ color: 'var(--text-muted)' }}>{labels[type]}</p>
